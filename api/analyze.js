...
Return this exact JSON structure, with Chinese user-facing content:
{
  "score": 0,
  "match": 0,
  "diagnosis": "",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json(normalizeReport(parsed, profile));
  } catch (error) {
    return res.status(500).json({
      error: error?.message || "Analyze API failed",
